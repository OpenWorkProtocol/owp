# Open Work Protocol (OWP) — 1.0-rc3

Status: **release candidate, not a production-readiness claim.** RC3 supersedes
RC2 as the current development candidate; RC2 remains the last published
release. RC2 previously closed public-release audit findings by adding an
explicit cancellation exit, bounded HTTP request handling, and replay-safe
mutation retries. RC3 preserves those core semantics and adds the optional
Software Work Integrity Profile. §4 states the intended 1.0 compatibility
contract; until 1.0 is released, an RC may still change to close a demonstrated
...
and `owp-desk` (personal administration). They are independent operator experiences, **not
...
**§1.3 — Human decisions should require minimal operator effort.** Every agent
question MUST carry options and evidence sufficient to decide from the card alone
(§12.1). The obligation is deliberately asymmetric: machine clients assemble the
available evidence instead of shifting that work to the operator.
...
As work completes, the active surface should shrink rather than accumulate history `[all]`.
...
Reference deployments include strongly time-oriented cases—publication schedules,
certificate expiry, and periodic backups—without adding calendar semantics to the
surface. Order is structural position and `depends_on`; clocks live in clients, and
periodic work is recurring **creation**, not recurrence `[publish][ops]`.
...
For software engineering work, OWP MAY compose with the normative optional
**Software Work Integrity Profile** in
[`annex-software-work-integrity-1.0-rc3.md`](annex-software-work-integrity-1.0-rc3.md).
That profile keeps Git, build systems, validators, attestations, and payment
rails outside the surface while standardizing the portable evidence boundary:
an agreed Definition of Done, an attempt-specific immutable source snapshot,
a provider delivery claim, independent validation bound to the exact result,
and validator-attested handoff. A core surface does not evaluate those objects;
it round-trips them through ordinary envelopes and records the clients' acts.

The boundary is intentionally similar to MCP's contract/host separation:
...
| Leaves host/product behavior to ecosystem implementations | Same: the examples are benchmarks, not the product |
...
| `deliverable.kind` | that it exists and is a flat label | feature/bug `[code]`; chapter/scene `[publish]`; study/thread `[research]`; upgrade/verify `[ops]`; claim/errand `[desk]` | five disjoint sets without surface interpretation |
...
| `rank_tiebreak` (§11) | the filter, the fixed tiers, and that one tiebreak may be declared | `unblocks-others` `[code][ops]`; `link-number` over manuscript position `[publish]`; none `[research]` | one deployment omits the slot, demonstrating optionality |
...
Three corollaries supported by the cross-domain examples:

- **Core vocabulary belongs in the spec only when every deployment domain needs it.** v0.2 named
  `git` in a core derivation; a deployment domain with no repositories exposed the mismatch `[publish]`.
- **An example may extend inside an envelope freely; it MUST NOT grow
  ...
  envelope is protocol feedback, not deployment-specific protocol machinery.
...
```json
{ "key": "PAY", "name": "payments", "goal": "One operator-level paragraph.",
...
{ "ref": "PAY-4", "project": "PAY",
  ...
  "intent": "One operator-level line.",
...
```json
{ "protocol": { "version": "1.0-rc3", "supported": ["1.0-rc3", "1.0-rc2"] },
...
| `session.heartbeat()` | Lightweight liveness; SHOULD piggyback on every verb |
...
The author's registration status previously controlled two independent concerns:
the evidence obligation and the routing direction. A creator client demonstrates why
those concerns must remain separate `[publish]`: a machine watcher should still supply
options and evidence under §12.1 while being able to direct its question to a human
operator. **The obligation stays attached to the author's class; only the routing
moved.** A surface MUST honour a stated `direction` in both places the routing appears:
the attention queue (§15) and the assignment packet (§10).
...
this is the point where the protocol requires a concise structured record.
...
1. `work.complete(ref, record)` moves the item to `review`, releases ownership,
   ...
   only infer it from event history `[desk]`. Recording the author makes the
   rule directly checkable.
...
answer `none`. Retrieval strategy—grep, full-text search, embeddings, or another mechanism—is a
deployment concern. Specifying it here would turn the protocol into a search-engine
contract.
...
operator-triaged diffs. A well-synchronized project produces an empty diff, so the
size of the reconciliation diff is a useful drift signal. A long-running agent can be
onboarded through the same operation.
...
- **`attention({project?, limit?, cursor?})`** → `{rows, cursor, total, totals,
  ...
  three fields would force every operator client to perform an N+1 read.
  ...
  **`totals` breaks the count down by kind** — `{decision, triage, health,
  ...
  costs it a `GROUP BY` and avoids repeated client-side queries. That
  ...
  **`policies` carries the standing rules for an act that cannot be undone.** A
  ...
  packet; the operator's own projections did not carry the same rules. For an
  irreversible operator decision, that made the row incomplete under §1.3 even
  though the agent assignment contained the necessary policy context `[ops]`. `triage`, `answer` and `unpark` are irreversible; `release`
  ...
  **informational**: a client MUST NOT offer it as an operator action and
  ...
  forward, so nothing an operator client offers will clear that row. A reference client previously mapped such a row to `release`, which removed a
  claim held by an active agent `[code]`. Clients branch on `action`; `reason` is prose for a person
  ...
  **The order MUST be total, and totality is a property of rows, not of
  ...
  This affects paging correctness. A surface that orders on (kind, age, target)
  alone has no order *within* a tie, so a cursor naming only those three can
  resolve repeatedly to the same row while other rows remain unreachable. A
  reference implementation exhibited this behavior with six open questions on
  one item: only one distinct row appeared across twenty-one pages `[research]`.
  The total-order requirement prevents that failure mode.
  ...
  **`cursor` MUST be a place, not a position.** It is an **opaque token**
  ...
  were cleared, and the operator never learns which `[desk][code]`. Because attention rows are routinely cleared between reads, an offset cursor
  does not preserve the required paging semantics. An opaque token also leaves the surface
...
- **`portfolio({project?})`** — per project: working-set size, in-flight items
  ...
  **`blocked`** (any dependency not completed). The surface already computes
  this for §11's hard filter; omitting it caused deployments with staged work
  to implement repeated reads and divergent interpretations of "blocked"
...
- **`work.view(ref, cursor?, limit?, order?)`** — the operator-side item read:
  ...
  bounded worklog is taken from; the page itself stays chronological. A bounded read must support the newest end because operator views commonly need
  recent activity. Reference clients otherwise fetched the full log and discarded
  older entries locally `[desk][code]`.
  ...
  own queue — but this array originally exposed both directions without distinguishing them.
  In one deployment an analyst-authored question was routed back into the analyst
  decision view without options `[research]`. Carrying `direction` makes the
  routing explicit to clients.
...
- **HTTP+JSON is the normative binding annex** — [`annex-http-1.0-rc3.md`](annex-http-1.0-rc3.md):
...
- **Authorization — the protocol's part**, because OWP has asymmetric roles.
  ...
  - **Agent authority**: sessions, work, events, own-claim release, discovery.
    ...
    own, so it needs the grant like anyone else. A surface that tests "is this a different session?" instead of "does this actor own
    this claim?" can allow a credential that omits the session header to release a
    claim it does not own; the reference implementation previously exhibited this
    behavior `[code]`.
...
- **Grants.** A grant is agent authority **plus named verbs under an optional
  ...
  Two rules make the model coherent; both are supported by reference implementation
  failures observed during development `[ops][desk]`:
  ...
  - **Grants compose: union across grants, intersection within one.** A client
    ...
    silently voids the rest. Within a single grant, **every field a grant names MUST
    hold** on every path by which the verb can be reached. A surface that checked
    `trigger_prefix` but ignored `project` broadened both the janitor and cron grants
    beyond their intended scope `[code]`.
- **The delegation invariant (normative):**
  ...
  itself `[code]`. Three properties are security-critical:
  ...
  - **It binds the act, not the verb.** A decision that resolves a question and
    ...
    checks only one verb path leaves equivalent decision paths unprotected and does
    not enforce the invariant consistently. The reference implementation previously
    guarded only one such path `[code]`.
  - **Proposals are output.** A watcher whose role is producing proposals must not
    also approve those same proposals through a grant; doing so would remove the
    independent decision boundary `[research]`.
...
A deployment that declares the Software Work Integrity Profile is additionally
conformant to that profile only when its participating clients and independent
validator satisfy Annex B. Core OWP conformance alone MUST NOT be represented
as proof of exact-source or build provenance.
...
RC3 supersedes RC2 as the current development candidate. It preserves the
RC2 core state machine and verb semantics and adds a normative optional Software
Work Integrity Profile for exact-source contracts, four-party Definition of Done
consensus, attempt-specific execution commitments, independent validation, and
validator-attested handoff. RC2 remains the last published release until the
RC3 reference implementations and independent interop evidence are upgraded.
Detailed compatibility and migration information is maintained in
