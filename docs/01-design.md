...
What never landed anywhere: subagent 2's nine-item internal checklist. It died with the
...
- **Transport**: MCP, plugin, skill, plain HTTP API, or several. Likely a small HTTP
  API with thin per-tool adapters (Claude Code gets a skill, others get theirs)
  generated from one definition — unproven.
- **Status-line staleness**: `status_line` is owner-maintained, which resolves the
  "rollup without an LLM per read" question — but a stale status line lies. Likely
  answer: staleness is derivable (last edit older than recent `progress` events →
  flag), keeping it in the detection rung.
- **Reground merge depth**: how much of the diff can be auto-accepted (pure additions?)
  versus always operator-triaged. Start fully triaged; loosen with evidence.
- **`review` as a state** versus a derived flag.
- **What the operator view looks like** once it is not assumed to be a board. Undecided
  on purpose — the schema comes first.
